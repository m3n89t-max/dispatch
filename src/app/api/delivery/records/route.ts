...
function parseCustomerName(cn: string): { driverName: string; vehicleNo: string; customerLabel: string } {
  const m = cn.match(/^(.+) \((.+)\)$/)
  if (m) return { driverName: m[1], vehicleNo: m[2], customerLabel: cn }
  return { driverName: cn, vehicleNo: '', customerLabel: cn }
}
